...
function App() {
    ...
    const [todosFromUrl, setTodosFromUrl] = useState([])

    const urlTodos = "https://jsonplaceholder.typicode.com/users/1/todos";


    const tmpTodos = useTodos(urlTodos);
    useEffect(() => {
        setTodosFromUrl(tmpTodos);
    }, [tmpTodos])
    console.log(todosFromUrl);

    // useEffect(() => {
    //     const urlTodos = "https://jsonplaceholder.typicode.com/users/1/todos";
    //     async function getTodosFromUrl() {
    //         let response = await fetch(urlTodos);
    //         let data = await response.json()
    //         setTodosFromUrl(data);
    //     }
    //     getTodosFromUrl()
    // }, [])
